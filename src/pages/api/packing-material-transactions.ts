...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const collection = db.collection('packingmaterialtransactions');

    switch (req.method) {
      case 'GET':
        // Add date range filtering
        const { startDate, endDate, materialId, type: queryType } = req.query;
        let query: any = {};
      ...
      case 'POST':
        const { materialId: postMaterialId, type = 'add', quantity, price = 0, date, note } = req.body;
        ...
        // Update packing material stock - use the quantity as is
